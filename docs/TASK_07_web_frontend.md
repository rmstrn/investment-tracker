...
| **Slice 7a+7b** | Landing + Pricing: `(marketing)/` route group, `/` landing (hero + 3 pillars + trust strip, anon render / authed redirect), `/pricing` (3 tiers aligned to `04_DESIGN_BRIEF §13.1`), Subscribe CTAs stubbed (Stripe in 7c / TD-057), 8 new Vitest specs | ✅ merged — PR #58, squash `528333b`, 2026-04-21. Opened TD-080 (paywall trigger wiring deferred). |
| **Slice 4a** | Manual Accounts CRUD: `(app)/accounts` route + list + Add/Rename/Delete для `connection_type=manual`, sidebar activation, 4 TanStack hooks (`useAccounts`, `useCreateAccount`, `useUpdateAccount`, `useDeleteAccount`), packages/ui `SyncStatusBadge` 'manual' variant, 3 Vitest smokes (50/50). Broker OAuth (4b/4c) блокирован TD-046. | ✅ merged — PR #59, squash `c5590f5`, 2026-04-21. Opened TD-079 (FK CASCADE vs handler soft-delete mismatch, P3, defense-in-depth). |
| **Slice 5a** | Transactions UI (add/edit/delete для `buy/sell/dividend` на Position Detail): single `TransactionFormDialog` с mode create|edit (diff-only PATCH), row-level kebab gated на `source === 'manual'` (API-sourced rows immutable per OpenAPI contract), 3 TanStack mutation hooks (`useCreateTransaction` / `useUpdateTransaction` / `useDeleteTransaction`) с invalidation `['position-transactions', id]` + `['portfolio']` + `['positions']`, `mapTransactionMutationError` (409 `DUPLICATE_TRANSACTION` / 403 `FORBIDDEN` tailored copy), 8 Vitest smoke (64/64 total). DateTime picker = `<input type="datetime-local">` fallback (no shadcn Calendar). Split/transfer/fee типы disabled в select с "Coming soon" suffix — реализация в Slice 5b. TD-081 reservation не использован (genuine debt не обнаружен). **Manual MVP end-to-end flow замкнут.** | ✅ merged — PR #60, squash `5e556a9`, 2026-04-21. 2 ADR в DECISIONS.md (form-dialog pattern + Idempotency-Key auto-inject rationale). |
| Slice 5b+ | Split / transfer / fee Transactions (ждёт TD-065), Insights feed (`/ai/insights/*`, ждёт TD-070), FloatingAiFab activation (context-aware mini-chat), Accounts 4b/4c (broker OAuth — TD-046), Settings, Paywall triggers (TD-080) + Stripe Checkout (Slice 7c), PWA, scope-cut header UI (`X-Partial-Portfolio` / `X-FX-Unavailable`), sidebar disabled-state для placeholder nav-слотов, mid-stream reconnect (TD-049), `ChatContext` deep-links из Positions | ⏳ pending |

Детали merge — `merge-log.md` (PR #45, PR #48, PR #50, PR #58, PR #59, PR #60 entries).
