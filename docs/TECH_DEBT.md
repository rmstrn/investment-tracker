...
**Priority:** P3.
**Source:** `PnlSparklineAnimated.tsx` (complexity 29) and `DividendCalendarAnimated.tsx` (complexity 28 after Slice-LP3.3 simplification — was 30 pre-3.3) exceed Biome max (15). Both are animated SVG components with inline conditional rendering, animation phase logic and prefers-reduced-motion branches that inflate cognitive complexity.
...
**Priority:** P2.
**Source:** Clerk `await auth()` + `redirect('/dashboard')` stripped from `(marketing)/page.tsx` to allow static first-pass deploy on staging without Clerk runtime. Signed-in users hitting the marketing landing will see it instead of redirecting.
**Desired state:** Restore `await auth()` guard when the marketing landing transitions to provedo.ai production — or when Clerk env vars are confirmed available in Vercel deploy env.
**Owner:** CC (web).
**Trigger:** provedo.ai migration slice (post-PO comparison review of first-pass landing).

### TD-092 — full v1.4 token migration across apps/web (replace pre-rebrand tokens)

**Added:** 2026-04-25 (Slice-LP1 Provedo first-pass landing).
**Priority:** P3.
**Source:** `(marketing)/layout.tsx` uses inline CSS variables (`--provedo-bg-page`, `--provedo-accent`, etc.) scoped to marketing route only, to avoid touching pre-rebrand tokens (`bg-background-primary`, `text-text-primary`, `ring-brand-500`) in `(app)/*` and `(auth)/*` routes. Full migration spec lives in `packages/design-tokens/MIGRATION_PROVEDO_v1.4.md`.
...
**Added:** 2026-04-25 (Slice-LP1 Provedo first-pass landing).
**Priority:** P3.
**Source:** `MarketingHeader.tsx` renders «Provedo» as inline `<span>` with Inter Semibold tracking-tight. `packages/ui/src/brand/Logo.tsx` still references pre-rebrand SVG. SVG wordmark for Provedo = product-designer follow-up artefact (separate slice per kickoff §2.17).
