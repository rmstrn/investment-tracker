...
> - **Δ4 — Dual-side validation pattern (architect, explicit out-of-scope here).** Architectural blessing of dual-side validation: Pydantic AI-side (in `apps/ai/`) for cheap pre-emit structural validation; Zod FE-side (this slice) for canonical structural + cross-field math validation at the trust boundary. **This slice owns ONLY the Zod side.** The Pydantic mirror is a separate slice (`SLICE-AI-CHARTS-V1`, post-merge) that consumes the same OpenAPI-derived contract via `zod-to-openapi` codegen. Source-of-truth policy: Zod schema in `packages/shared-types/src/charts.ts` is canonical; Pydantic models are generated, not hand-authored. Cross-field invariants (waterfall conservation Δ2, sum-to-total Δ1) live ONLY in Zod and are NOT duplicated in Pydantic. Structural exclusions (Risk Flags 1/2/3) DO live in both runtimes. Document in JSDoc on each schema export. See architect ADR §«Brainstorm-pass addendum / Δ4».
> - **T-8 — Cross-currency `dailyChangeBasis` discriminator (finance, MVP-IN-SCOPE).** Treemap tiles for multi-currency portfolios have a unit ambiguity: `tile.dailyChangePct` can mean local-currency price change OR base-currency total change (including FX). **Right-Hand decision: include in MVP schema.** Add `treemap.dailyChangeBasis: z.enum(['local', 'base'])` as a payload-level required field (not per-tile). Cost: one enum field + one parser test. Benefit: ambiguity closed at the structural layer instead of via prompt-engineering convention. Renderer caption then resolves to either «Color = today's price change in local currency» or «Color = today's price change in your base currency, including FX». See finance audit §9 Δc.
> - **S-2 retag (finance) — V2-deferred-gate.** S-2 (scatter `referenceLines.label` prescriptive vocab regex) is mooted for MVP because Δ3 already removes Scatter from the union; the Zod parse fails before the regex would fire. NO MVP schema work required for S-2. Re-activates as a precondition for V2 scatter re-introduction (legal-advisor sign-off gate). Backend has nothing to implement here.
...
- **`packages/shared-types/src/charts.ts`** — new file containing:
  ...
  - **Cross-currency basis discriminator on Treemap (finance T-8 — MVP-in-scope):** add `dailyChangeBasis: z.enum(['local', 'base'])` as a required payload-level field on `TreemapPayload` (not per-tile — mixing within a treemap is incoherent). Schema requires the field on every treemap payload; no default. Renderer (FE kickoff) consumes this to compose the caption. Parser test: `TreemapPayload.parse(payloadWithoutBasis)` fails; both `'local'` and `'base'` values parse successfully. See finance audit §9 Δc.
...
- **Vitest schema unit tests** — `packages/shared-types/src/charts.test.ts`:
  ...
  - **T-8 — Treemap `dailyChangeBasis` discriminator:** valid treemap payload with `dailyChangeBasis: 'local'` parses; same with `'base'` parses; treemap payload omitting the field fails parse with explicit Zod issue path `['dailyChangeBasis']`. Invalid value (`'mixed'`, `'usd'`, etc.) fails parse.
...
- [ ] `TreemapPayload` requires `dailyChangeBasis: z.enum(['local', 'base'])` (T-8); payload omitting the field fails parse; both enum values parse successfully.
