...
**DEPLOY TARGET CORRECTED 2026-04-25 by PO (Navigator update):** target is **staging.investment-tracker.app** (existing staging environment), NOT provedo.ai. All references к «provedo.ai» в этом kickoff to be read as «staging.investment-tracker.app». DNS already configured, deploy pipeline exists (Railway или Fly.io — devops-engineer confirms на дисптче). No domain setup needed для this slice.
...
**One-line goal:** Ship a static, copy-locked Provedo landing page deployed at `https://provedo.ai` so PO can do head-to-head comparison vs Claude Design first-pass.
...
- provedo.ai domain — sunk cost, controlled by PO с 2026-04-23. Reuse для first-pass review без second domain-purchase. provedo.ai migration — отдельный slice post-review.

**Acceptance criteria** (full checklist в §4 ниже).

**What is "done"**
- `https://provedo.ai` resolves к Provedo landing over HTTPS
...
2. **Provedo→Provedo brand string sweep** в `(app)/*` routes (dashboard, chat, insights, positions, accounts) и `(auth)/*` routes — НЕ трогать.
...
5. **provedo.ai DNS / migration** — domain owned, но миграция waits на post-review PO call. provedo.ai — first-pass deploy target.
...
**ВАЖНО — текущий `apps/web/src/app/(marketing)/page.tsx` уже существует** и содержит Provedo-era landing (Hero / ThreePillars / TrustStrip + Clerk redirect). Strategy:

- **REPLACE** existing `page.tsx` content fully (Provedo copy → Provedo copy).
...
**Single route:** `apps/web/src/app/(marketing)/page.tsx` (replace Provedo landing)
...
**SEO / meta:** Update existing `metadata` export in `page.tsx` per `landing-provedo-v1.md` §7. Use `og:url = https://provedo.ai/` (NOT provedo.ai/) — first-pass deploys на provedo.ai, og должен match deployed URL. canonical URL same.
...
- Existing global tokens (`bg-background-primary`, `text-text-primary`, etc.) point at v1.3 Provedo slate-violet system. Touching them = full migration scope.
- Inline scope в `(marketing)/layout.tsx` обеспечивает Provedo visual без касания Provedo tokens, dashboard, chat, etc.
...
- NO purple / violet anywhere (Provedo-era killed по PO calibration)
...
- **Domain:** `provedo.ai` (DNS controlled by PO since 2026-04-23 sunk-cost purchase)
...
provedo.ai — bare apex domain.

**For Vercel:**
1. In Vercel project settings → Domains → Add `provedo.ai` + `www.provedo.ai`
2. At PO's domain registrar (Namecheap / Cloudflare / wherever provedo.ai lives):
   - Apex `provedo.ai` → A record pointing к Vercel anycast IP `76.76.21.21`
   - `www.provedo.ai` → CNAME → `cname.vercel-dns.com`
3. Wait DNS propagation (5-30 min typical, до 48hr worst case)
4. Verify HTTPS: `curl -I https://provedo.ai` should return 200 + valid Let's Encrypt cert

**Action item:** devops-engineer needs **DNS access credentials** for provedo.ai — check с PO какой регистрар и есть ли у devops доступ. Если нет — PO performs DNS step manually with devops-provided records.

### 3.4 Environment variables / secrets

**None required.** Landing is fully static. No backend API calls. No Clerk runtime secrets used (Clerk redirect happens server-side, no signed-in users will hit provedo.ai для first-pass since traffic = PO + reviewers only).
...
**Recommendation:** Option 3 — frontend-engineer removes `auth()` + `redirect('/dashboard')` for landing; document как TD-0XX «restore Clerk redirect when provedo.ai transitions to provedo.ai production». Trigger: provedo.ai migration slice.
...
- [ ] `https://provedo.ai` returns 200, valid HTTPS, valid TLS cert
- [ ] `http://provedo.ai` redirects к HTTPS
- [ ] `https://www.provedo.ai` redirects к apex (or vice versa, consistent canonical)
...
- DNS unset provedo.ai (return к `<empty>` или previous parking page) → instant rollback. TTL <24hr.
...
- [ ] SEO meta + OG verbatim из §7 (но `og:url = https://provedo.ai/` для first-pass — see §2.3 outscope override)
...
- [ ] `https://provedo.ai` resolves, valid HTTPS, page renders
- [ ] `http://` → `https://` redirect works
- [ ] Lighthouse on provedo.ai ≥90 mobile (real-world incl. CDN latency)
...
T2+15m: devops-engineer deploys from main → provedo.ai
...
**Risk:** Frontend-engineer encounters Provedo-era tokens (`bg-background-primary`, `text-text-primary`, `ring-brand-500`) и решает «better just migrate everything для consistency». Single-PR scope blows up к 14-step Phase A-E migration.
...
- If frontend-engineer encounters пример где inline scope не работает (e.g. `MarketingHeader` shared by /pricing route), file TD-entry, не fix inline. Pricing route может остаться Provedo-era для first-pass — PO not visiting `/pricing` для landing comparison.

### Risk 2 — provedo.ai domain confusion (LOW impact, accepted by PO)

**Risk:** Users hitting provedo.ai see «Provedo» branding. SEO crawlers index provedo.ai с Provedo content. Brand confusion if anyone outside PO+reviewer team finds URL.
...
**SEO mitigation:** add `<meta name="robots" content="noindex,nofollow">` к landing для provedo.ai stage. Remove when migrating к provedo.ai. This adds zero visual cost и prevents accidental indexing.
...
### Risk 4 — Clerk auth() runtime failure on provedo.ai (MEDIUM probability, MEDIUM impact)

**Risk:** existing `(marketing)/page.tsx` has `await auth()` from Clerk. Без Clerk runtime initialized в Vercel deploy, this throws.

**Mitigation:** §3.4 Option 3 — strip Clerk redirect from landing. Anonymous-only first-pass. File TD: «restore Clerk redirect when provedo.ai → provedo.ai prod migration».
...
**Risk:** `packages/ui/src/components/Logo.tsx` still says «Provedo» (or «Investment Tracker») — visible в `MarketingHeader`.
...
- **Provedo era preservation:** Existing Provedo landing (`page.tsx` original content) preserved в git history at SHA `6811815`. Easy restore if needed.
- **Pricing page (`/pricing`):** out-of-scope. May still show Provedo copy. PO not reviewing pricing для first-pass comparison.
...
5. **DNS access for provedo.ai:** does devops-engineer have credentials to PO's domain registrar? If no, PO performs DNS step manually with devops-provided records.
6. **Robots noindex on provedo.ai:** confirm OK to add `<meta name="robots" content="noindex,nofollow">` для first-pass stage. Recommend YES.
...
- Replace (marketing)/page.tsx Provedo hero/pillars/trust → Provedo Hero/DemoTabs/Insights/Aggregation/RepeatCTA/Footer
...
- Add merge-log entry: SHA <new>, Slice-LP1 merged, deploy target provedo.ai
...
- Update docs/PO_HANDOFF.md §current state: landing on provedo.ai, ready for PO/Claude Design comparison
```
...
- [ ] DNS access to provedo.ai confirmed (or PO available для manual DNS step)
- [ ] Vercel project created, linked к GitHub repo (or staged для manual import)
- [ ] provedo.ai + www.provedo.ai added к Vercel project domains
...
**Devops-engineer:**
- Deployment URL: https://provedo.ai
