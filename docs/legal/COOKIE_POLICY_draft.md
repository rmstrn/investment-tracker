...
This Cookie Policy describes how Provedo (operated by **`[ENTITY TBD]`**, "we") uses cookies and similar technologies on `provedo.ai` and related properties (collectively, the "Service"). It should be read together with our [Privacy Policy](./PRIVACY_POLICY_draft.md) and [Terms of Service](./TOS_draft.md).
...
| `provedo_csrf` | Provedo | CSRF token for form submissions | Session | First-party, HTTP-only, SameSite=Strict |
...
| Name | Set by | Purpose | Duration | Type |
|---|---|---|---|---|
| `provedo_currency` | Provedo | Store selected display currency | 1 year | First-party |
| `provedo_theme` | Provedo | Store UI theme preference (light/dark/system) | 1 year | First-party |
| `provedo_locale` | Provedo | Store preferred language | 1 year | First-party |
...
Most browsers offer a "Do Not Track" (DNT) setting. There is no industry consensus on how sites should respond to DNT signals. Provedo does **not** currently respond to DNT signals; instead, we rely on the cookie consent banner for opt-in control of non-essential cookies. For Global Privacy Control (GPC) signals under CCPA, we honor GPC for California residents.
...
Настоящая Политика cookies описывает использование cookies и аналогичных технологий на `provedo.ai` и связанных ресурсах («Сервис») компанией Provedo (оператором является **`[ЮРЛИЦО — TBD]`**). Читается вместе с [Privacy Policy](./PRIVACY_POLICY_draft.md) и [Terms of Service](./TOS_draft.md).
...
| `provedo_csrf` | Provedo | CSRF-токен | Сессия | first-party, HTTP-only, SameSite=Strict |
...
| Имя | Устанавливается | Цель | Длительность | Тип |
|---|---|---|---|---|
| `provedo_currency` | Provedo | Валюта отображения | 1 год | first-party |
| `provedo_theme` | Provedo | Тема UI | 1 год | first-party |
| `provedo_locale` | Provedo | Предпочтительный язык | 1 год | first-party |
...
Provedo **не** реагирует на сигнал Do Not Track; опираемся на cookie-баннер. Для Global Privacy Control (GPC) — чтим GPC для резидентов Калифорнии по CPRA.
