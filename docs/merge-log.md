...
## SLICE-CHARTS-FE-V1 — 10 MVP chart components + showcase wiring

**Branch:** `chore/plugin-architecture-2026-04-29` (consolidated PR pending — FE lands inside the chore branch alongside backend; QA follows).
**Implementation SHA:** TBD (chore branch local; not yet on `main`).
**Date:** 2026-04-29
**Scope.** Implemented 10 MVP chart components in `packages/ui/src/charts/` consuming the typed `ChartPayload` discriminated union from SLICE-CHARTS-BACKEND-V1. T1 eager: `LineChart`, `AreaChart` (rewrite), `BarChart` (rewrite), `DonutChart` (rewrite), `Sparkline`, `Calendar` (CSS-grid), `Treemap` (Recharts native). T2 lazy via `React.lazy()`: `StackedBar`, `Waterfall` (with `computeWaterfallSteps` named export for QA unit tests). T3 lazy: `Candlestick` — created and exported from `lazy.ts` but NO showcase demo (legal sign-off gate). Scatter excluded per architect Δ3. Shared internals `_shared/`: `formatters.ts` (Intl-driven currency/percent/count/ratio + compact thresholds), `useReducedMotion.ts` (SSR-safe), `useChartKeyboardNav.ts`, `buildTooltipProps.ts`, `ChartDataTable.tsx` (visually-hidden a11y transcript per CHARTS_SPEC §7.4), `ChartSkeleton.tsx` (per-kind shapes), `ChartEmpty.tsx`, `ChartError.tsx` (`?debug=1` payload reveal gate). Tokens rewrite: `SERIES_VARS` (7-hue palette as `var(--chart-series-N)` strings), `CHART_TOKENS`, `colorForTreemapChange`, `labelOnTile`, `CHART_ANIMATION_MS=600`, `DONUT_ORDER`. CSS vars declared in `apps/web/src/app/globals.css` light defaults on `:root`, dark overrides on both `.dark` and `[data-theme="dark"]` per architect ADR theme decision. `packages/ui/package.json` adds `./charts/lazy` export entry. Showcase wiring at `/design#charts` renders all 10 MVP types from CHARTS_SPEC §5.3 fixtures (T2 behind `<Suspense>`, Candlestick demo SKIPPED, Scatter absent). Mandatory baked captions: Waterfall (finance audit C6), drift Bar (B-2 / B8), Treemap FINRA + T-8 cross-currency basis. LineChart series-7-on-dark-solo auto-swap → series-1 with localhost-gated dev `console.warn` per CHARTS_SPEC §2.3. A11y baseline on every chart: `role="img"`, auto-generated `aria-label` from `meta.title|alt`, `aria-describedby` to hidden `<ChartDataTable>`, `tabIndex={0}`. `prefers-reduced-motion: reduce` honoured: `isAnimationActive={!prefersReducedMotion}` on every Recharts root.
**Verification.** `pnpm --filter @investment-tracker/ui typecheck` green. `pnpm --filter @investment-tracker/ui test` 17/17 green (10 instantiation + 10 ChartDataTable snapshots + 5 caption/transform/dark-swap + 2 Sparkline). `pnpm --filter @investment-tracker/ui lint` clean. `pnpm --filter @investment-tracker/web typecheck` green. `pnpm --filter @investment-tracker/web test` 69/69 green (incl. migrated `position-price-chart` consumer). `pnpm --filter @investment-tracker/web build` green: `/design` route `9.71 kB` (was `9.28 kB`); `/design` First-Load JS `284 kB` (was `261 kB`) → +23 kB gz delta, well under the 50 kB cap. Apps/web build emits no chart-related warnings.
**TDs opened:** TD-094 (P3, trigger: dashboard / positions chart consumer migration — `apps/web/src/components/positions/position-price-chart.tsx` uses `as unknown as` cast on data prop because `MultiSeriesPoint.catchall(z.number())` rejects literal-typed mixed-key objects; revisit when shared-types adds a fixture-helper export). TD-095 (P3, trigger: design-tokens migration to v1 Provedo cream-paper / cocoa-board palette — chart-series CSS vars currently declared inline in `apps/web/src/app/globals.css` rather than `packages/design-tokens/tokens/semantic/{light,dark}.json`; lift to design-tokens DTCG when SLICE-DSM-V1 lands).
**Migrations:** none. The only existing chart consumer (`position-price-chart.tsx`) was migrated inline to the new `payload`-driven API; tests still pass via the same `aria-label` accessor.
