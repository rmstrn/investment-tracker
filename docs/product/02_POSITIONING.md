# 02 — Product Positioning (LOCKED v3.1 — Provedo + imperative hero + Second Brain as tagline)

**Owner:** `navigator` agent (strategy core).
**Framework:** April Dunford's "Obviously Awesome" (5-step).
**Date locked (v1):** 2026-04-22 · **Date locked (v2 — Second Brain metaphor):** 2026-04-23 · **Date locked (v3 — imperative hero restored, Second Brain demoted to tagline):** 2026-04-23 · **Date locked (v3.1 — product name = Provedo):** 2026-04-23
**Status:** LOCKED (v3.1). PO locked 7 decisions from Option 4 review synthesis 2026-04-23 (see `DECISIONS.md` «Option 4 review synthesis: 7 PO decisions locked» + `STRATEGIC_OPTIONS_v1.md` v1.6 LOCK section). v3 repositions Option 4 as tagline + brand-world copy; hero reverts to imperative. v3.1 locks product name as **Provedo** (see `DECISIONS.md` 2026-04-23 «Product name LOCKED: Provedo; domain provedo.ai»). Body narrative synced to imperative-hero + tagline-only framing + Provedo-as-named-agent in same commit. Future edits require PO sign-off through Navigator.
...
- **Insights = what your second brain surfaces proactively.** Weekly digest of dividends, drawdowns, events, concentration shifts — things you would miss.
- **Coach = what your second brain notices about your behavior over time.** Pattern-reads on actual trade history («you sold Apple at the local low three times last year»).

Tertiary surfaces fit without forcing: scenarios (second brain runs what-ifs), explainer (inline vocabulary unpacking), aggregation (second brain holds everything in one place).
...
Archetype applies to a named product (**Provedo**) with a tagline («Second Brain for Your Portfolio»), not to the metaphor as product-identity. The name carries the Magician+Sage weight (Latin root, quiet invention); the tagline carries the category-claim work; the imperative hero («Ask your portfolio») carries Everyman directness.

v1 archetype was «Magician + Everyman». v2 pulled Sage forward (knowledge-worker metaphor) while keeping Everyman as a warmth modifier. v3 (2026-04-23) clarifies: the Magician+Sage balance attaches to the named product Provedo, not to «Second Brain» as a product identity — demotion from hero + product-name to tagline is reflected in archetype framing.
...
- Provedo uses an imperative voice in user-facing copy («Ask your portfolio»), with «Second Brain for Your Portfolio» as tagline + brand-world copy. The product name (Provedo) carries the memory-semantics; copy does not need to repeat «your second brain» as narrative hero-voice on every line.
...
- NOT advisor — information/education only (Lane A LOCKED 2026-04-23; positive trust signal: «we are not selling you anything»)
- NOT broker (no trading execution) — *functional fact, not landing hero angle*
- NOT analyst-in-a-box (we do not produce «buy NVDA» takes; we do not rate stocks)
...
Each tab shows a mock chat interaction demonstrating one capability of the second brain. Content is illustrative; content-lead owns final copy.

| Tab | Situation user types | Mock response shown | Which surface |
|---|---|---|---|
| **Ask** | "Why am I down this month?" | Breakdown: 62% of drawdown — Apple (-11%) + Tesla (-8%). Chart. Sources (AAPL Q3 earnings, TSLA Q3 earnings). | Chat |
| **Analyze** | "How diversified am I?" | Tech 58% (above US retail median 34%). Financials 22%, healthcare 8%, other 12%. Pie chart. | Chat → tool |
| **Notice** | *(weekly push)* "3 things this week" | (1) $124 in dividends · (2) NVDA at 52w high — 14% of your portfolio · (3) EUR cash losing -2.1% to inflation. | Insights |
| **Project** | "What if the dollar drops 10%?" | Portfolio is 87% USD assets → -8.7% in EUR terms. Exposed: S&P ETF ($12K), Apple ($8K). Hedges: EU equities, gold, EUR bonds. | Chat → scenario tool |

**Note:** Coach (pattern-reading on trade history) and Explainer (inline vocabulary) are not standalone tabs — Coach surfaces inside Notice (weekly push), Explainer appears inline inside any response. Five surfaces in four tabs — honest product, unified metaphor.
...
> Provedo не является зарегистрированным инвестиционным советником. Информация предоставляется в образовательных целях. Прошлая доходность не гарантирует будущую. Все инвестиционные решения вы принимаете самостоятельно.
...
1. **Trust over beauty** — fintech is about reliability. Zero «magic», everything explainable. Provedo's memory claim is only as strong as its ability to cite every output — hallucinations break the promise.
2. **Newcomer should not feel stupid** — all terms explained inline in context. The Everyman modifier in Provedo's archetype mix attaches here.
3. **Privacy and security at financial-app level** — Provedo holds your data; it is yours, not shared.
4. **Read-only broker connection** — never request trading rights. Provedo observes; it does not act.
5. **AI shows sources for every claim/analysis** — a portfolio brain that cannot cite is just guessing.
6. **Mobile and web are equal citizens** — Provedo is with you everywhere.

## Onboarding promise (progressive disclosure, Option 4 specific)

Onboarding stages reinforce the memory metaphor through what the product does (remembers, notices, reads patterns), with Provedo as the named agent rather than «your second brain» as narrative persona:

- **Stage 1 (immediate, minute 1):** Chat + aggregation visible. «Provedo remembers what you hold after first sync.»
- **Stage 2 (within 24h of first sync, or earlier if SnapTrade backfill surfaces pattern-readable history):** First insight drops. «Provedo noticed the first thing.»
- **Stage 3 (day 30, or earlier via backfill-derived first-value moment, after enough history accumulates):** First coach pattern-read. «Provedo has read enough of your trades to see a pattern.»
...
- **vs Notion / Obsidian (cross-category)**: second brain for knowledge / notes vs our second brain for one specific knowledge domain — deep domain-expertise in finance, not a generic memory container.

## Risks acknowledged (see also `STRATEGIC_OPTIONS_v1.md` Option 4 risks section)

- **Abstract-metaphor hero cognition** ~4 sec vs Oracle's tested 3 sec. Landing must earn the extra second via sub-proof specificity.
- **Coach 30-day cold-start** inherited from Option 3. Hero promises «remembers» but first month is empty if onboarding mitigation is not designed. Product-designer dispatch handles this; `tech-lead` feasibility check scopes it.
- **Eng scope heaviest of four options** — all three surfaces must ship credibly at MVP. Commodity drift to «AI tracker with chat + coach bolted on» if coach does not land cleanly.
- **Metaphor import risk for ICP B** — «second brain» is Notion / Obsidian-coded; may read as pretentious for AI-native newcomers.
- **Naming territory conflict** — brand-strategist round 5 must find a viable candidate in mind / memory / knowledge / synapse space that avoids the four rejected rounds (Ask imperatives, Folio roots, evocative trio, Ask creative spellings).
